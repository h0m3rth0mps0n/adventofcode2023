...
//        List<String> input = Util.getResourceLines("2023/day5.txt");
    ...
    public static void part2v2(List<Long> seedInput, MapperChain mapperChain) {
        List<Range> seedRanges = new ArrayList<>();
        for(int idx = 0, len = seedInput.size(); idx < len; idx+=2) {
            seedRanges.add(new Range(seedInput.get(idx),
                    seedInput.get(idx),
                    seedInput.get(idx + 1)));
        }

        Mapper initialMapper = new Mapper(seedRanges);

        initialMapper.splitRanges(mapperChain.getMappers().get(0));
    }
